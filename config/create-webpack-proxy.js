...
const cookieTransform = require('./cookie-transform');
...
function createInsightsProxy({
  ...
  const esi =
    ...
    new ESI({
      ...
      cache: false,
      onError: (src, error) => {
        console.error(`An error occurred while resolving an ESI tag`);
        console.error(error);
      },
    });
  ...
  return {
    ...
    proxy: [
      ...
      ...(localChrome
        ? [
            {
              ...
              onProxyReq: (_proxyReq, req, res) => {
                ...
                proxyVerbose &&
                  console.log('serving locally', req.url, '--->', newPath, '------>', localPath[0], '\n\n');
        ...
        : []),
      ...(proxyRoutes
        ? Object.entries(proxyRoutes).map(([route, redirect]) => {
