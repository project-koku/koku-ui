...
cat <<- EEOOFF
    ...
    Use scripts/release/release-onprem.sh to assemble $ONPREM_BRANCH from $MAIN_BRANCH
...
confirmOnpremDirectMerge()
{
  ...
  echo "*** Use scripts/release/release-onprem.sh to pin those UIs to their latest prod tags."
  echo ""
  ...
  case $YN in
    [Yy]* ) return 0;;
    [Nn]* | "" )
      echo "\n*** Aborting. Use scripts/release/release-onprem.sh to assemble from prod tags."
      exit 0
      ;;
    * )
