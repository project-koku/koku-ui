...
interface GroupBySubOrgOwnProps {
...
interface GroupBySubOrgState {
  currentItem?: string;
  defaultItem: string;
  isGroupByOpen: boolean;
}

interface GroupBySubOrgOption extends SelectOptionObject {
  id?: string;
}

type GroupBySubOrgProps = GroupBySubOrgOwnProps & InjectedTranslateProps;

class GroupBySubOrgBase extends React.Component<GroupBySubOrgProps> {
  protected defaultState: GroupBySubOrgState = {
    defaultItem: this.props.groupBy || this.props.options[0].value,
    isGroupByOpen: false,
  };
  public state: GroupBySubOrgState = { ...this.defaultState };

  constructor(props: GroupBySubOrgProps) {
    super(props);
  ...
  public componentDidUpdate(prevProps: GroupBySubOrgProps) {
  ...
  private handleGroupBySelect = (event, selection: GroupBySubOrgOption) => {
  ...
  public render() {
    ...
    const selection = groupByItems.find(
      (item: GroupBySubOrgOption) => item.id === currentItem
    );
...
const GroupBySubOrg = translate()(GroupBySubOrgBase);

export { GroupBySubOrg, GroupBySubOrgProps };
