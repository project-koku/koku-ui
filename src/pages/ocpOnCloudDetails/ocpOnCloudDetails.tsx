...
import { getQuery, OcpOnCloudQuery, parseQuery } from 'api/ocpOnCloudQuery';
...
interface OcpOnCloudDetailsStateProps {
  providers: Providers;
...
interface OcpOnCloudDetailsDispatchProps {
  fetchReport: typeof ocpOnCloudReportsActions.fetchReport;
}

interface OcpOnCloudDetailsState {
  columns: any[];
  isExportModalOpen: boolean;
  rows: any[];
  selectedItems: ComputedOcpOnCloudReportItem[];
}

type OcpOnCloudDetailsOwnProps = RouteComponentProps<void> &
  InjectedTranslateProps;

type OcpOnCloudDetailsProps = OcpOnCloudDetailsStateProps &
  OcpOnCloudDetailsOwnProps &
  OcpOnCloudDetailsDispatchProps;

const reportType = OcpOnCloudReportType.cost;

const tagKey = 'or:tag:';

const baseQuery: OcpOnCloudQuery = {
  delta: 'cost',
  filter: {
    limit: 10,
    offset: 0,
...
class OcpOnCloudDetails extends React.Component<OcpOnCloudDetailsProps> {
  protected defaultState: OcpOnCloudDetailsState = {
    columns: [],
    isExportModalOpen: false,
    rows: [],
    selectedItems: [],
  };
  public state: OcpOnCloudDetailsState = { ...this.defaultState };
  ...
  public componentDidUpdate(
    prevProps: OcpOnCloudDetailsProps,
    prevState: OcpOnCloudDetailsState
  ) {
  ...
  private getExportModal = (computedItems: ComputedOcpOnCloudReportItem[]) => {
  ...
  private getRouteForQuery(query: OcpOnCloudQuery, reset: boolean = false) {
    // Reset pagination
    if (reset) {
      query.filter = {
        ...query.filter,
  ...
  private handleGroupByClick = groupBy => {
    const { history, query } = this.props;
    const groupByKey: keyof OcpOnCloudQuery['group_by'] = groupBy as any;
    const newQuery = {
  ...
  private handleSelected = (selectedItems: ComputedOcpOnCloudReportItem[]) => {
    this.setState({ selectedItems });
  };
  ...
  public render() {
    ...
    return (
      <div className={css(styles.ocpOnCloudDetails)}>
...
>((state, props) => {
  ...
  const reportError = ocpOnCloudReportsSelectors.selectReportError(
    state,
    reportType,
    queryString
  );
...
const mapDispatchToProps: OcpOnCloudDetailsDispatchProps = {
