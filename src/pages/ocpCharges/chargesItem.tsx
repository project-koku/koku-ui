...
import { styles } from './ocpCharges.styles';
...
interface ChargesItemStateProps {
  report?: Report;
  reportFetchStatus?: FetchStatus;
}

interface ChargesItemDispatchProps {
  fetchReport?: typeof reportsActions.fetchReport;
}

type ChargesItemProps = DetailsItemOwnProps &
  ChargesItemStateProps &
  ChargesItemDispatchProps &
...
class ChargesItemBase extends React.Component<ChargesItemProps> {
  public state: State = {
    expanded: false,
  };
  ...
  public componentDidUpdate(prevProps: ChargesItemProps) {
        ...
        additionalInfo={[
          <ListView.InfoItem key="1" stacked>
            ...
            <span>
              {(Boolean(item.deltaValue > 0) &&
                ((Boolean(date < 31) &&
                  t('ocp_charges.increase_since_date', {
                    date,
                    month,
                    value,
                  })) ||
                  t('ocp_charges.increase_since_last_month', {
                    date,
                    month,
                    value,
                  }))) ||
                (Boolean(item.deltaValue < 0) &&
                  ((Boolean(date < 31) &&
                    t('ocp_charges.decrease_since_date', {
                      date,
                      month,
                      value,
                    })) ||
                    t('ocp_charges.decrease_since_last_month', {
                      date,
                      month,
                      value,
                    }))) ||
                t('ocp_charges.no_change_since_date', { date, month })}
            </span>
          </ListView.InfoItem>,
        ]}
        actions={[
...
const ChargesItem = translate()(connect()(ChargesItemBase));

export { ChargesItem, ChargesItemBase, ChargesItemProps };
