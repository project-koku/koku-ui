...
test('formats value', () => {
  ...
  expect(
    screen.getByText(
      '{value} {units} ({percent} %){"percent":"10","units":"{units, select, byte_ms {Byte-ms} cluster_month {cluster-month} core {core} core_hours {core-hours} gb {GB} gb_hours {GB-hours} gb_mo {GB-month} gb_ms {GB-ms} gib {GiB} gibibyte_month {GiB-month} hour {hours} hrs {hours} ms {milliseconds} pvc_month {PVC-month} vm_hours {VM-hours} other {{units}}}{\\"units\\":\\"units\\"}","value":100}'
    )
  ).not.toBeNull();
