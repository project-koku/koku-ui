...
describe('getComputedReportItems', () => {
  test('undefined report returns an empty chart data', () => {
    expect(
      getComputedReportItems({ report: undefined, idKey: 'instance_type' })
    ).toEqual([]);
  });
  ...
  test('convert report items into chart format', () => {
    ...
    expect(getComputedReportItems({ report, idKey: 'instance_type' })).toEqual(
      expected
    );
  });
  ...
  test('set label to alias if given when label key is account', () => {
    ...
    expect(getComputedReportItems({ report, idKey: 'account' })).toEqual(
      expected
    );
  });
});
