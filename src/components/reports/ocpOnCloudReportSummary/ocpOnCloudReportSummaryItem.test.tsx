...
} from './ocpOnCloudReportSummaryItem';
import { styles } from './ocpOnCloudReportSummaryItem.styles';

const props: OcpOnCloudReportSummaryItemProps = {
  formatOptions: {},
...
xtest('formats value', () => {
  shallow(<OcpOnCloudReportSummaryItem {...props} />);
...
test('gets percentage from value and total value', () => {
  const view = shallow(<OcpOnCloudReportSummaryItem {...props} />);
...
test('sets percent to 0 if totalValue is 0', () => {
  const view = shallow(
    <OcpOnCloudReportSummaryItem {...props} totalValue={0} />
  );
  expect(view.find(Progress).props().value).toMatchSnapshot(
    'Progress Bar Value'
  );
