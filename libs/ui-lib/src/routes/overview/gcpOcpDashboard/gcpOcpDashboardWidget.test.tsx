...
import { GcpOcpDashboardWidget, getIdKeyForTab } from './gcpOcpDashboardWidget';
...
jest.mock('store/dashboard/gcpOcpDashboard', () => ({
  __esModule: true,
  GcpOcpDashboardTab: { gcpProjects: 'gcpProjects', regions: 'regions', services: 'services' },
...
describe('gcpOcpDashboardWidget connector', () => {
  test.each([
    [GcpOcpDashboardTab.gcpProjects, 'gcp_project'],
    [GcpOcpDashboardTab.regions, 'region'],
    [GcpOcpDashboardTab.services, 'service'],
  ])('getIdKeyForTab %#', (tab, expected) => {
    expect(getIdKeyForTab(tab as any)).toBe(expected);
  });

  test('maps currency into base widget', () => {
    const { getByTestId } = render(<GcpOcpDashboardWidget widgetId={1} /> as any);
