...
jest.mock('components/sourceDetail/CredentialForm', () => ({
  CredentialForm: () => <div data-testid="credential-form" />,
}));

const mockedGetSource = getSource as jest.MockedFunction<typeof getSource>;
const mockedPauseSource = pauseSource as jest.MockedFunction<typeof pauseSource>;
const mockedResumeSource = resumeSource as jest.MockedFunction<typeof resumeSource>;
...
describe('SourceDetail', () => {
  ...
  it('shows "Source not found" when fetch fails', async () => {
  ...
  it('shows "Paused" label and warning alert for a paused source', async () => {
    ...
    expect(screen.getByRole('heading', { level: 1, name: 'AWS Source' })).toBeInTheDocument();
  ...
  it('calls handleCheckAvailability when the refresh button is clicked', async () => {
    ...
    const refreshBtn = screen.getByRole('button', { name: 'Check source availability' });
  ...
  it('handles handleTogglePause error gracefully', async () => {
    const user = userEvent.setup();
    mockedGetSource.mockResolvedValue(activeSource);
    mockedPauseSource.mockRejectedValue(new Error('fail'));
    await renderDetail('uuid-1', jest.fn(), true);
