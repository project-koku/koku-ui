...
test('test fetch price list action', async () => {
  ...
  await store.dispatch(actions.fetchRate(pUuid));
  expect(fetchMock).toHaveBeenCalled();
});

test('default state', async () => {
  const store = createStore();
  expect(selectors.getRateState(store.getState())).toMatchSnapshot();
});

test('from init to fetch request', async () => {
  const store = createStore();
  expect(selectors.status(store.getState(), pUuid)).toBe(undefined);
  expect(selectors.getRateState(store.getState(), pUuid)).toMatchSnapshot();
  store.dispatch(actions.fetchRateRequest({ providerUuid: pUuid }));
  expect(selectors.getRateState(store.getState())).toMatchSnapshot();
});
...
test('from fetch request to fetch success', async () => {
  ...
  store.dispatch(actions.fetchRateRequest({ providerUuid: pUuid }));
  expect(selectors.getRateState(store.getState())).toMatchSnapshot();
  store.dispatch(actions.fetchRateSuccess(ratesMock, { providerUuid: pUuid }));
  expect(selectors.getRateState(store.getState(), pUuid)).toMatchSnapshot();
});
...
test('from fetch request to fetch error', async () => {
  ...
  store.dispatch(actions.fetchRateRequest({ providerUuid: pUuid }));
  expect(selectors.getRateState(store.getState(), pUuid)).toMatchSnapshot();
  store.dispatch(actions.fetchRateFailure(Error('Opps!'), { providerUuid: pUuid }));
  expect(selectors.getRateState(store.getState(), pUuid)).toMatchSnapshot();
});
