...
import { selectPriceListError, selectPriceListFetchStatus } from './priceListSelectors';
...
export function fetchPriceList(
  ...
  return (dispatch, getState) => {
    ...
    const status = selectPriceListFetchStatus(state, priceListType, priceListQueryString);
...
export function updatePriceList(priceListType: PriceListType, uuid?: string, payload?: PriceListPayload): ThunkAction {
  return (dispatch, getState) => {
    const state = getState();
    const fetchStatus = selectPriceListFetchStatus(state, priceListType, undefined);
