...
describe('priceList store', () => {
  ...
  test('update request, success, and failure', () => {
    ...
    expect(selectors.selectPriceListFetchStatus(makeRoot(state), type, '')).toBe(FetchStatus.inProgress);
    state = priceListReducer(
      state,
      updatePriceListSuccess({} as any, { fetchId: fid, notification: { t: 1 } } as any)
    );
    expect(selectors.selectPriceListFetchStatus(makeRoot(state), type, '')).toBe(FetchStatus.complete);
    expect(selectors.selectPriceListNotification(makeRoot(state), type, '')).toEqual({ t: 1 });
    state = priceListReducer(state, updatePriceListFailure({} as any, { fetchId: fid, notification: { e: 1 } } as any));
    expect(selectors.selectPriceListError(makeRoot(state), type, '')).toEqual({});
    expect(selectors.selectPriceListNotification(makeRoot(state), type, '')).toEqual({ e: 1 });
  });
