...
test('getQuery filter_by props are converted', () => {
  expect(getQuery(query)).toEqual('filter%5Bsubscription_guid%5D=test&order_by%5Bsubscription_guid%5D=asc');
});

test('getQueryRoute filter_by props are not converted', () => {
  expect(getQueryRoute(query)).toEqual('filter_by%5Bsubscription_guid%5D=test&order_by%5Bsubscription_guid%5D=asc');
});
