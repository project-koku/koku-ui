...
jest.mock('components/i18n', () => ({ __esModule: true, getLocale: () => 'en', intl: { formatNumber: (v: number, opts?: any) => new Intl.NumberFormat('en', opts).format(v), formatMessage: (_m: any, v: any) => `${v.symbol || ''}${v.value || ''}` } }));
