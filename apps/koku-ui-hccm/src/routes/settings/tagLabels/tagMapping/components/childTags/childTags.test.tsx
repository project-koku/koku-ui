...
jest.mock('./childTagsTable', () => ({
  ChildTagsTable: ({ isLoading, onSelect, onSort }: any) => (
    <div>
...
jest.mock('./childTagsToolbar', () => ({
  ChildTagsToolbar: ({ onBulkSelect, onFilterAdded, onFilterRemoved, pagination }: any) => (
    <div>
      {pagination}
...
describe('ChildTags', () => {
  ...
  const renderPage = () =>
    render(
      <Provider store={createStore(() => ({}))}>
        <ChildTags onBulkSelect={onBulkSelect} onSelect={onSelect} selectedItems={[{ uuid: 'c-1' }]} />
      </Provider>
    );
  ...
  test('renders and bulk selects the page', async () => {
    ...
    expect(onBulkSelect).toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ uuid: 'c-2' })]));
