...
import { NoSelectionsState } from 'routes/settings/costModel/costModelBreakdown/priceLists/components/state';
...
const TimelineChart: React.FC<TimelineChartProps> = ({ isReset, onResetClick, priceLists }) => {
  ...
  const priceListDatum = priceLists?.map(item => {
    ...
    return {
      ...
      x: item.priority ? `${item.priority} ${item.name}` : item.name,
      y0: startDate,
      y: endDate,
    };
  });
