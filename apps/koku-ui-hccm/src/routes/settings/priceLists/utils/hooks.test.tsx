...
import { updatePriceListFailure, updatePriceListRequest, updatePriceListSuccess } from 'store/priceLists/priceListActions';

import { usePriceListAddNotification, usePriceListDuplicate, usePriceListEnabledToggle } from './hooks';
...
describe('priceList hooks', () => {
  ...
  test('usePriceListAddNotification fires notification when update completes with notification payload', async () => {
  ...
  test('usePriceListAddNotification does not add a toast when isNotificationEnabled is false', async () => {
    const addNotification = jest.fn();
    const {
      useAddNotification,
    } = require('@redhat-cloud-services/frontend-components-notifications/hooks') as { useAddNotification: jest.Mock };
    ...
    renderHook(() => usePriceListAddNotification(false), {
      wrapper: wrapperFor(store),
    });
