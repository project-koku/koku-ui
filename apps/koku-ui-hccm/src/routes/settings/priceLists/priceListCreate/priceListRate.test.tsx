...
jest.mock('routes/settings/priceLists/priceListBreakdown/rates', () => ({
  RateTable: (props: { rates?: { custom_name?: string }[] }) => (
  ...
  RateToolbar: (props: { pagination?: React.ReactNode }) => (
...
jest.mock('routes/settings/priceLists/priceListBreakdown/rates/components/add', () => ({
...
describe('PriceListRate', () => {
    ...
    render(
      wrap(
        <IntlProvider defaultLocale="en" locale="en">
          <PriceListRate canWrite priceList={{ currency: 'USD', rates: [], uuid: 'pl-new' } as any} />
        </IntlProvider>
      )
    );
  ...
  test('changing page shows next slice of rates (client-side)', async () => {
    ...
    render(
      wrap(
        <IntlProvider defaultLocale="en" locale="en">
          <PriceListRate canWrite priceList={{ currency: 'USD', rates: many as any[], uuid: 'pl' } as any} />
        </IntlProvider>
      )
    );
    expect(await screen.findByRole('button', { name: /^add rate$/i })).toBeInTheDocument();
