...
import { priceListReducer, priceListSelectors, priceListStateKey } from 'store/priceLists';
...
describe('PriceListActions', () => {
  ...
  test('opens delete modal from kebab', async () => {
    renderActions(<PriceListActions canWrite isDeleteAction priceList={basePl} />);
  ...
  test('deprecated list shows restore and calls toggle hook', async () => {
    ...
    renderActions(
      <PriceListActions canWrite isDeprecateAction onDeprecate={onDeprecate} priceList={{ ...basePl, enabled: false }} />
    );
    fireEvent.click(screen.getByRole('button', { name: /more options/i }));
  ...
  test('duplicate menu invokes onDuplicate from hook mock', async () => {
    ...
    renderActions(<PriceListActions canWrite isDuplicateAction onDuplicate={onDuplicate} priceList={basePl} />);
    fireEvent.click(screen.getByRole('button', { name: /more options/i }));
  ...
  test('delete modal cancel invokes onClose', async () => {
    ...
    renderActions(<PriceListActions canWrite isDeleteAction onClose={onClose} priceList={basePl} />);
    fireEvent.click(screen.getByRole('button', { name: /more options/i }));
    fireEvent.click(await screen.findByRole('menuitem', { name: /delete price list/i }));
  ...
  test('deprecate modal cancel invokes onClose', async () => {
    const onClose = jest.fn();
    renderActions(<PriceListActions canWrite isDeprecateAction onClose={onClose} priceList={basePl} />);
    fireEvent.click(screen.getByRole('button', { name: /more options/i }));
  ...
  test('delete modal confirm dispatches remove and completes', async () => {
    ...
    renderActions(<PriceListActions canWrite isDeleteAction onDelete={onDelete} priceList={basePl} />, store);
    fireEvent.click(screen.getByRole('button', { name: /more options/i }));
    fireEvent.click(await screen.findByRole('menuitem', { name: /delete price list/i }));
    const dialog = screen.getByRole('dialog');
    ...
    await waitFor(() =>
      expect(
        priceListSelectors.selectPriceListFetchStatus(store.getState() as any, PriceListType.priceListRemove, '')
