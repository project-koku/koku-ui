...
describe('DeprecatePriceListModal', () => {
  jest.useRealTimers();
  ...
  test('confirm dispatches update with enabled false, required name, and completes in the store', async () => {
    ...
    render(
      <Provider store={store}>
        <IntlProvider defaultLocale="en" locale="en">
          <DeprecatePriceListModal isOpen priceList={{ uuid: 'u-dep', name: 'To deprecate', enabled: true } as any} />
        </IntlProvider>
      </Provider>
    );
    ...
    await waitFor(() =>
      expect(
        priceListSelectors.selectPriceListFetchStatus(store.getState() as any, PriceListType.priceListUpdate, '')
