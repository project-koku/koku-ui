...
const DeletePriceListModal: React.FC<DeletePriceListModalProps> = ({ isOpen, onClose, onDelete, priceList }) => {
  const dispatch: ThunkDispatch<RootState, any, AnyAction> = useDispatch();
  const intl = useIntl();

  const priceListType = PriceListType.priceListRemove;

  const { priceListUpdateStatus } = useMapToProps({ priceListType });
