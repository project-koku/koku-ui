...
import type { TagValueExt } from 'routes/settings/priceList/components/rates/utils';
...
const GpuTagValues: React.FC<GpuTagValuesProps> = ({
  ...
  return (
    <Table aria-label={intl.formatMessage(messages.costModelsEnterTagRate)} borders={false} variant={'compact'}>
      <Thead noWrap>
        <Tr>
          <Th aria-label={intl.formatMessage(messages.equalsSymbol)}></Th>
          <Th>
          ...
          <Th aria-label={intl.formatMessage(messages.actionsColumn)}></Th>
        </Tr>
      </Thead>
      <Tbody>
