...
interface ConfirmEditRateModalOwnProps {
  isOpen?: boolean;
  onClose?: () => void;
  onContinue?: () => void;
...
type ConfirmEditRateModalProps = ConfirmEditRateModalOwnProps;

const ConfirmEditRateModal: React.FC<ConfirmEditRateModalProps> = ({ isOpen, onClose, onContinue, priceList }) => {
  const intl = useIntl();
  ...
  return (
    <Modal className="costManagement" isOpen={isOpen} onClose={onClose} variant={ModalVariant.medium}>
      <ModalHeader title={intl.formatMessage(messages.editRateTitle)} />
      <ModalBody>
        <Stack hasGutter>
          <StackItem>
      ...
      <ModalFooter>
        <Button isAriaDisabled={!isConfirmChecked} onClick={onContinue} variant="primary">
