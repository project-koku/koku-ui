...
interface ImpactReviewModalOwnProps {
  isOpen?: boolean;
  onClose?: () => void;
  onSuccess?: () => void;
...
type ImpactReviewModalProps = ImpactReviewModalOwnProps;

const ImpactReviewModal: React.FC<ImpactReviewModalProps> = ({ isOpen, onClose, onSuccess, priceList }) => {
  const intl = useIntl();
  ...
  return (
    <Modal className="costManagement" isOpen={isOpen} onClose={onClose} variant={ModalVariant.medium}>
      <ModalHeader title={intl.formatMessage(messages.priceListImpactReview)} />
      <ModalBody>
        <Stack hasGutter>
          <StackItem>
      ...
      <ModalFooter>
        <Button isAriaDisabled={!isConfirmChecked} onClick={onSuccess} variant="primary">
