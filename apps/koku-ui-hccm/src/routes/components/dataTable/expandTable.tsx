...
interface ExpandTableState {
  expandedRows?: Set<any>;
  rows?: any[];
}

type ExpandTableProps = ExpandTableOwnProps & RouterComponentProps & WrappedComponentProps;

class ExpandTable extends React.Component<ExpandTableProps, ExpandTableState> {
  public state: ExpandTableState = {
  ...
  public render() {
    const { columns, gridBreakPoint = 'grid-2xl', intl, isActionsCell, isAllExpanded, isLoading, rows } = this.props;
    const { expandedRows } = this.state;
...
export default injectIntl(withRouter(ExpandTable));
