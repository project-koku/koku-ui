...
import OptimizationsContainersTable from './optimizationsContainersTable';
...
jest.mock('./optimizationsContainersDataTable', () => ({
  OptimizationsContainersDataTable: ({ onSort }: { onSort?: (type: string, asc: boolean) => void }) => (
    <div>
...
jest.mock('./optimizationsContainersToolbar', () => ({
  OptimizationsContainersToolbar: ({ onFilterAdded, onFilterRemoved, pagination }: any) => (
    <div>
      {pagination}
...
const renderTable = (props: Record<string, unknown> = {}, entries?: any[]) =>
  render(
    <MemoryRouter initialEntries={entries || ['/']}>
      <OptimizationsContainersTable queryStateName="table" {...props} />
    </MemoryRouter>
  );

describe('OptimizationsContainersTable', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  test('restores containers query state from location', () => {
  ...
  test('applies top-level filter_by when containers state is missing', () => {
    mockSelectors({ meta: { count: 2, limit: 10, offset: 0 } }, FetchStatus.complete, undefined);
    renderTable(
      {},
      [
