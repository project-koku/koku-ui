...
jest.mock('api/queries/query', () => ({ __esModule: true, parseQuery: () => ({ group_by: { account: ['redhat'] } }) }));
...
describe('EmptyFilterState extra', () => {
	...
	test('route-driven non-array group_by value triggers ItemScroll', async () => {
		jest.isolateModules(() => {
			...
			jest.doMock('api/queries/query', () => ({ __esModule: true, parseQuery: () => ({ group_by: { account: 'koku' } }) }));
	...
	test('route-driven group_by with no matches renders default icon', () => {
		jest.isolateModules(() => {
			...
			jest.doMock('api/queries/query', () => ({ __esModule: true, parseQuery: () => ({ group_by: { account: ['other'] } }) }));
			const C = require('./emptyFilterState').default;
			render(<C />);
	...
	test('route-driven group_by array with early break after first non-match then match', () => {
		jest.isolateModules(() => {
			...
			jest.doMock('api/queries/query', () => ({ __esModule: true, parseQuery: () => ({ group_by: { account: ['nomatch', 'redhat'] } }) }));
			const C = require('./emptyFilterState').default;
			render(<C />);
			expect(document.querySelector('img')).toBeTruthy();
		});
	...
	test('route-driven non-array group_by value redhat shows item1 path', () => {
		jest.isolateModules(() => {
			...
			jest.doMock('api/queries/query', () => ({ __esModule: true, parseQuery: () => ({ group_by: { account: 'redhat' } }) }));
			const C = require('./emptyFilterState').default;
			render(<C />);
			expect(document.querySelector('img')).toBeTruthy();
		});
	...
	test('route-driven multiple group_by keys with match in second key', () => {
		jest.isolateModules(() => {
			...
			jest.doMock('api/queries/query', () => ({ __esModule: true, parseQuery: () => ({ group_by: { account: 'nomatch', project: ['koku'] } }) }));
			const C = require('./emptyFilterState').default;
			render(<C />);
			expect(document.querySelector('img')).toBeTruthy();
		});
