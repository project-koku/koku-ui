...
jest.mock('components/i18n', () => ({ __esModule: true, intl: { formatDateTimeRange: (s: Date, e: Date) => `${s.getFullYear()}-${s.getMonth()+1}-${s.getDate()} to ${e.getFullYear()}-${e.getMonth()+1}-${e.getDate()}`, formatMessage: (_m: any, v: any) => JSON.stringify(v), formatRelativeTime: (v: number, unit: string) => `${v} ${unit}` } }));
