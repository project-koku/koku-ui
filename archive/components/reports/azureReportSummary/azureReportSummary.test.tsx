...
import {
  AzureReportSummary,
  AzureReportSummaryProps,
} from './azureReportSummary';

const props: AzureReportSummaryProps = {
  title: 'report title',
...
test('on fetch status complete display reports', () => {
  const view = mount(
    <AzureReportSummary {...props}>hello world</AzureReportSummary>
  );
  expect(view.find(CardBody).text()).toEqual('hello world');
});

test('show subtitle if given', () => {
  const view = mount(<AzureReportSummary {...props} subTitle={'sub-title'} />);
...
test('show details link in card footer if given', () => {
  const view = mount(
    <AzureReportSummary {...props} detailsLink={<a href="#">link</a>} />
  );
